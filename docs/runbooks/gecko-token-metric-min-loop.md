...
before manual interruption about 11h32m after start, before the planned 12H
plus post-run window. The immediate minimum-loop impact is a larger
Metric-zero backlog: Token / Metric / Notification /
...
Remaining Metric-pending Red, 2026-07-09: the approved 168h
`--onlyMetricPending --noNotificationCapture --write` command ran once for
ids `8440..8489`. It returned `selected=50`, `ok=50`, `written=50`,
`skipped=0`, `error=0`, and `providerErrorCount=0`. Metric ids `2882..2931`
were created and all selected rows moved to `metricsCount=1`.

Token / Metric / Notification / HolderSnapshot moved `4266 / 1757 / 32 / 1
-> 4266 / 1807 / 32 / 1`; only Metric changed. Notification statuses stayed
`captured=27`, `sent=5`, selected-row Notification / HolderSnapshot totals
stayed `0 / 0`, and Telegram / checkpoint / scheduler-systemd / rawJson /
provider body side effects stayed `0`. The remaining bounded rehearsal queue
is still Metric-first: 168h `metricPending=80`, `enrichPending=180`,
`notifyCandidate=0`. Next step is another Green post-run review and
remaining Metric-pending preflight, not direct continuation.
...
Metric-one resnapshot preflight refresh, 2026-06-08: the preview path selected
a clean second-snapshot cohort. `sinceMinutes=10080` with `--onlyMetricOnce`
selected ids `8259..8210`, all `metricsCount=1`, with safe latest Metric ids
present, `latestMetricAgeMinutes=4231..4243`, Notification count `0`,
HolderSnapshot count `0`, `providerErrorCount=0`, and no fetch/write.
`sinceMinutes=1440` is now too narrow and selected `0`. If approved, use the
...
`7527..7478` from one Metric to two Metrics. The exact approved
...
are `2667..2716`, with observedAt range
`2026-06-09T09:25:10.363Z..2026-06-09T09:37:58.148Z`; all latest Metrics
have safe price, FDV, reserve, and top-pool presence.

Only Metric changed: Token / Metric / Notification / HolderSnapshot moved
`4065 / 1557 / 22 / 1 -> 4065 / 1607 / 22 / 1`, and global Metric buckets
are now `0=2748`, `1=1080`, `2+=237`. Notification capture remained disabled;
...
second Red, fallback command, or rawJson full dump occurred.

The post-run growth report evaluates `235` pumpOnly Metric>=2 rows and found
...
Metric-one growth review, 2026-06-10: the minimum-loop review confirms ids
`7477..7428` improved observation depth but did not add a meaningful growth
winner. All `50` target rows are Metric>=2, with missing first FDV / latest
FDV / reserve counts `0 / 0 / 0`. The best FDV multiple is `1.2494`; only
one row reached `>=1.1x`, and none reached `>=1.25x` or `>=2x`. Latest FDV is
`40` near flat, `8` down, and `2` up above `1.01x`.

The cohort score shape is `C/0=48` and `B/2=2`; B/2 max FDV is `1.0000`, and
there are no C/1 rows. Token id `7577` remains the sole clear growth winner.
Continue to treat Metric-one resnapshot as an observation-depth lane; another
...
The following Green review passed, and the next network-enabled limit `50`
Metric backlog continuation also succeeded. It selected ids `6966..6917`,
wrote Metric ids `2117..2166`, returned `selected=50`, `ok=50`,
`written=50`, `error=0`, `providerErrorCount=0`, and all provider error
categories `0`. All selected rows moved to `metricsCount=1`, while
Notification and HolderSnapshot counts stayed `0`. Counts moved only in
Metric: `3023 / 1007 / 22 / 1 -> 3023 / 1057 / 22 / 1`; rolling 168h
...
That Green review passed, and a fourth network-enabled limit `50` Metric
backlog continuation also succeeded. Preflight selected ids `6866..6859` plus
`6758..6717`; gap ids `6858..6759` already had Metrics and were correctly
excluded by `--onlyMetricPending`. The Red wrote Metric ids `2217..2266`,
returned `selected=50`, `ok=50`, `written=50`, `error=0`,
`providerErrorCount=0`, and all provider error categories `0`. All selected
rows moved to `metricsCount=1`, while Notification and HolderSnapshot counts
stayed `0`. Counts moved only in Metric:
`3023 / 1107 / 22 / 1 -> 3023 / 1157 / 22 / 1`; rolling 168h
...
That Green review passed, and another small network-enabled limit `10`
enrich/rescore continuation succeeded. It selected ids `7048..7039`, all of
which had one GeckoTerminal Metric and no Notification or HolderSnapshot
rows. The safe alias exact command ran once without `--notify` and returned
`selected=10`, `ok=10`, `error=0`, `enrichWriteCount=10`,
`rescoreWriteCount=10`, `contextWriteCount=10`,
`metaplexAttemptedCount=10`, `metaplexAvailableCount=0`,
...
The selected rows moved from `mint_only` to `partial` with enriched/rescored
timestamps, reviewFlags, and GeckoTerminal context. Their Metrics remained
`2035..2044`; no new Metric rows were written. One row, id `7042`, became
`hardRejected=true` with safe reason `Matched HARD_NG: scam`; one row is
score `C / 1`, and the remaining rows are score `C / 0`. Notification /
Telegram, HolderSnapshot, Metric write, retry, auto-send, scheduler/systemd,
rawJson dump, and `pnpm smoke` stayed `0`. The next step should be Green
post-run enrich/report review for ids `7048..7039` before any additional Red.

That Green review passed, and another small network-enabled limit `10`
enrich/rescore continuation succeeded. It selected ids `7038..7029`, all of
which had one GeckoTerminal Metric and no Notification or HolderSnapshot
rows. The safe alias exact command ran once without `--notify` and returned
`selected=10`, `ok=10`, `error=0`, `enrichWriteCount=10`,
`rescoreWriteCount=10`, `contextWriteCount=10`,
...
The selected rows moved from `mint_only` to `partial` with enriched/rescored
timestamps, reviewFlags, and GeckoTerminal context. Their Metrics remained
`2045..2054`; no new Metric rows were written. No selected row became
hard-rejected; one row is score `C / 1`, and the remaining rows are score
`C / 0`. One row also gained Metaplex context. Notification / Telegram,
HolderSnapshot, Metric write, retry, auto-send, scheduler/systemd, rawJson
dump, and `pnpm smoke` stayed `0`. The next step should be Green post-run
enrich/report review for ids `7038..7029` before any additional Red.
