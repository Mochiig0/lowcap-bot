...
- The first foreground-created mint,
  ...
  present. The other foreground-created mint,
  `6MD8LtMX1Jf7W9hDs8rnthkeFS2sonzSaYiQHkZgpump`, remains untouched at
  `mint_only` with no name / symbol, no Metrics, and no latestMetric. The next
  Green gate for `5vLb...pump` is rawJson-free report confirmation for the two
  Metric rows.
...
- Always-on monitoring is still not implemented: there is no scheduler, queue
  ...
  confirmation for the two `5vLb...pump` Metric rows, a read-only preflight
  before `6MD8...pump` enrich/rescore, a detect tmux bounded preflight, or
