...
- The first foreground-created mint,
  ...
  systemd were not invoked during the Metric step. The other foreground-created
  ...
  Green gate for `5vLb...pump` is rawJson-free report confirmation.
...
- Always-on monitoring is still not implemented: there is no scheduler, queue
  ...
  for every Red step. After that, the next candidates are rawJson-free report
  confirmation for `5vLb...pump`, a read-only preflight before
  `6MD8...pump` enrich/rescore, a detect tmux bounded preflight, or formalizing
  metric snapshot tmux bounded operation as an interim entrypoint. Systemd
  stays on hold until a user-systemd-capable environment exists, and unbounded
  watch remains prohibited.
