...
- this single-mint loop did not send Telegram, did not update checkpoints, did not start watch mode, and did not touch systemd; it confirms the one-shot real-data path, minimal time-series append, and read-only history visibility before automation, while multi-token batches and watch/systemd operation remain unconfirmed
...
- Gecko detect always-on work has not started: the pump.fun one-shot write gate and same-mint one-shot observation loop with two Metrics are confirmed, including read-only report/compare visibility for that time series, but detect watch write, detect foreground/tmux operation, detect systemd operation, and metric snapshot watch write remain unconfirmed
...
- `metrics:report` is sufficient for confirming same-mint Metric history by `observedAt`, but its normal columns still do not expose a human-friendly safe summary for stored Gecko snapshot price / fdv / reserve / topPool fields; a future small improvement should add rawJson-free summary columns such as `priceUsdPresent`, `fdvUsdPresent`, `reserveUsdPresent`, and `topPoolPresent`
