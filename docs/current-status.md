...
- The detect foreground bounded watch wrapper has now passed its first bounded
  ...
  `6MD8...pump` through second Metric append.
- The first foreground-created mint,
  ...
  price / fdv / reserve / topPool presence were true. The next Green gate for
  `6MD8...pump` is rawJson-free report confirmation for the two Metric rows.
...
- Always-on monitoring is still not implemented: there is no scheduler, queue
  ...
  confirmation for the two `6MD8...pump` Metric rows, a detect tmux bounded
  preflight, or
