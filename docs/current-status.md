...
- Always-on monitoring is still not implemented: there is no scheduler, queue
  ...
  for every Red step. After that, the next candidates are another bounded
  detect candidate, or formalizing metric snapshot tmux bounded operation as an
  interim entrypoint.
