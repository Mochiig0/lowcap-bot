...
- the same mint has now reached read-only report/compare confirmation: `metrics:report -- --mint ... --limit 2` and `token:compare -- --mint ...` show both Metric `observedAt` values, while `token:show` and `tokens:compare-report` show latestMetric or cohort-level latestMetric summaries
- read-only cohort reporting has also been checked after the safe-summary-column update: `metrics:report -- --limit 10` shows multiple token / multiple Metric rows including the target mint's two rows, with `priceUsdPresent`, `fdvUsdPresent`, `reserveUsdPresent`, and `topPoolPresent` present on each row and without rawJson output
- `tokens:compare-report -- --source geckoterminal.new_pools --metadataStatus partial --hasMetrics true --minMetricsCount 2 --latestMetricSource geckoterminal.token_snapshot --limit 10` includes the target mint and shows `metricsCount=2`, latestMetric source, and latestMetric observedAt as a cohort-level latestMetric summary
- this single-mint loop and cohort reporting did not send Telegram, did not update checkpoints, and did not touch systemd; they confirm the one-shot real-data path, minimal time-series append, bounded single-mint watch write, bounded batch watch write with one eligible token, foreground bounded watch natural exit with minGap skip, tmux bounded watch natural exit with one append plus one minGap skip, read-only history visibility, and cohort report visibility before automation, while foreground append, two-or-more-token simultaneous Metric write, unbounded watch, restart-oriented operation, and systemd operation remain unconfirmed
...
- metric snapshot systemd preflight has been checked read-only after the tmux bounded gate, and the current wrapper / sample unit should not be started as-is: the wrapper defaults to `LOWCAP_GECKOTERMINAL_METRIC_LIMIT=5`, no `--pumpOnly`, no `--maxIterations`, `LOWCAP_GECKOTERMINAL_METRIC_INTERVAL_SECONDS=1800`, `LOWCAP_GECKOTERMINAL_METRIC_START_DELAY_SECONDS=900`, and the sample unit uses `Restart=always`, so it is broader than the confirmed bounded gate and could also leave CLI JSON output in journald
- Gecko detect always-on work has not started: the pump.fun one-shot write gate, same-mint one-shot observation loop with two Metrics, bounded single-mint and batch Metric snapshot watch writes, bounded foreground/tmux Metric snapshot gates, and read-only multi-token Metric cohort reporting are confirmed, but detect watch write, detect foreground/tmux operation, detect systemd operation, two-or-more-token simultaneous metric snapshot write, long-running metric snapshot watch, restart-oriented metric snapshot operation, and metric snapshot systemd operation remain unconfirmed
