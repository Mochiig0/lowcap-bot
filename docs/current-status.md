...
- `pnpm review:queue:geckoterminal` now also exposes small pending-age fields derived from `firstSeenSourceSnapshot.detectedAt` when present, otherwise `Token.createdAt`, plus lightweight enrich/metric pending age buckets (`<=5m`, `<=15m`, `<=60m`, `>60m`) for read-only lag reading under bounded follow-up runs
