...
Bounded write rehearsal Metric-pending snapshot Red, 2026-07-08:

- The repo-local Red execution Skill was applied, and the human-approved
  Metric-pending snapshot ran exactly once with the explicit 168h window:
  `pnpm -s metric:snapshot:geckoterminal -- --pumpOnly --limit 50 --sinceMinutes 10080 --minGapMinutes 60 --interItemDelayMs 15000 --onlyMetricPending --noNotificationCapture --write`.
...
- The command selected ids `8490..8539` from the bounded write rehearsal
  cohort and completed cleanly: `selectedCount=50`, `okCount=50`,
...
- Expected side effects were bounded to Metric writes. Token / Metric /
  Notification / HolderSnapshot moved `4266 / 1707 / 32 / 1 -> 4266 / 1757 /
  32 / 1`, so Metric delta was `+50` and Token / Notification /
  HolderSnapshot deltas were all `0`. Notification statuses stayed
  `captured=27`, `sent=5`, `failed=0`.
- Target rows ids `8490..8539` moved from `metricsCount=0` to
  `metricsCount=1`. New Metric ids are `2832..2881`, source
  `geckoterminal.token_snapshot`, observedAt range
  `2026-07-08T13:27:12.449Z..2026-07-08T13:40:03.010Z`, and safe
  market-data presence is price / FDV / reserve / topPool `50 / 50 / 50 /
  50`. Selected-row Notification and HolderSnapshot totals stayed `0 / 0`.
- Post-run 168h planner still has more bounded rehearsal cleanup work:
  `metricPending=130`, `enrichPending=180`, `staleReview=180`,
  `notifyCandidate=0`, failed Notification `0`, retry candidate `0`, and
  auto-send allowed candidate `0`. The next task should be a Green post-run
...
Interrupted-run guarded enrich Red, 2026-06-06:

- The repo-local Red safety Skill was applied and the approved
  network-enabled / out-of-sandbox guarded enrich command ran exactly once:
  `pnpm -s token:enrich-rescore:geckoterminal:safe -- --pumpOnly --limit 50 --sinceMinutes 720 --interItemDelayMs 15000 --onlyMetricCovered --write`.
  Expected HEAD `8ada526 docs: preflight guarded enrich after interrupted runner`
  matched and the working tree was clean before execution. Provider HEAD was
  reachable without body dump.
- Result: `selection.onlyMetricCovered=true`, `selected=29`, `ok=29`,
  `error=0`, `enrichWriteCount=29`, `rescoreWriteCount=29`,
  `contextWriteCount=29`, `metaplexAttemptedCount=29`,
  `metaplexAvailableCount=0`, `metaplexSavedCount=0`,
  `metaplexErrorKindCounts={metadata_account_missing:29}`,
  `rateLimited=false`, `rateLimitedCount=0`, `interItemDelayCount=28`,
  `notifyWouldSendCount=0`, and `notifySentCount=0`.
- The command selected ids `8259..8231`, not the full preflighted
  `8259..8210` range. Cause: the rolling 720 minute cutoff had advanced to
  `2026-06-05T04:23:21.343Z` by execution time, excluding ids `8230..8210`.
  No retry, second Red, fallback, or manual compensation command was run.
- Counts confirm only Token rows changed: Token / Metric / Notification /
  HolderSnapshot stayed `4065 / 1457 / 22 / 1`; metadata moved
  `mint_only=3083`, `partial=969`, `enriched=13` to `mint_only=3054`,
  `partial=998`, `enriched=13`; Metric buckets stayed `0=2748`, `1=1230`,
  `2+=87`; Notification statuses stayed `captured=17`, `sent=5`.
- Target aggregate for ids `8259..8210` after the Red: `partial=29`,
  `mint_only=21`, `metricsCount=1=50`, `score=C/0=50`,
  `hardRejectedCount=1`, `enrichedAt/rescoredAt/reviewFlags/scoreBreakdown`
  present for `29`, Notification total `0`, and HolderSnapshot total `0`.
  Actual selected ids `8259..8231` have GeckoTerminal context present
  `29 / 29` and Metaplex context `0 / 29`.
- Queue after: default 24h has `metricPending=632`, `enrichPending=653`,
  `staleReview=653`, `notifyCandidate=0`; rolling 12h has
  `metricPending=0`, `enrichPending=0`, `staleReview=0`,
  `notifyCandidate=0`; rolling 168h has `metricPending=792`,
  `enrichPending=873`, `staleReview=923`, `notifyCandidate=0`.
- Watchlist remains report-only: rolling 168h has `15` B/2 rows,
  `14` ready and `1` missing Metric. Notification / Telegram stayed closed:
  disabled/enabled auto-send allowed stayed `0 / 0`, retry candidate stayed
  `0`, failed Notification stayed `0`, and no Notification create/update/send
  or Telegram send occurred.
- This addressed `29` rows of the interrupted 12H detect-only backlog. The
  remaining ids `8230..8210` are still Metric-covered but mint-only; do not
  run another Red directly. If continuing cleanup, start with a fresh Green
...
Metric-one resnapshot preflight refresh, 2026-06-08:

- The new fetch-free `--onlyMetricOnce` preview is clean for a second-snapshot
  growth-detection lane. With `--pumpOnly --limit 50 --sinceMinutes 10080
  ...
  latest Metric age `4231..4243` minutes, `providerErrorCount=0`, and no
  provider fetch or DB write.
- Window check: `sinceMinutes=1440` is now empty, so the earlier 1440 minute
  Red command is no longer a useful target. Use `sinceMinutes=10080` for the
  next human-approved Red if the operator wants to move these rows from
  Metric-one to Metric>=2.
...
Metric-one growth review for ids `7477..7428`, 2026-06-10:

- Green read-only/docs-only review on HEAD
  `402cb5a docs: record metric once resnapshot` confirmed the global
  `metrics:growth-report` post-check again. Safety flags remained
  `readOnly=true`, `providerFetchExecuted=false`, `dbWriteExecuted=false`,
  `telegramSendExecuted=false`, and `rawJsonIncluded=false`; pumpOnly
  Metric>=2 evaluated rows stayed `285`, with buckets `0=1820`, `1=1021`,
  `2+=285`.
- Global growth interpretation is unchanged: top FDV/reserve remains
  `3.8445 / 3.7064`, FDV `2x/3x/5x/10x=1/1/0/0`, `C/1` remains the only
  2x+ score bucket, `B/2` max remains `1.0058`, and hardRejected 2x+ remains
...
- The target cohort has `50` rows and all are `metricsCount>=2`. Missing
  first FDV, latest FDV, and reserve counts are `0 / 0 / 0`. Cohort top
  `fdvMultiple` is `1.2494`; top reserve multiple is `1.0000`. FDV buckets
  `>=1.1/>=1.25/>=1.5/>=2/>=3/>=5/>=10` are `1/0/0/0/0/0/0`. Latest FDV
  shape is `down=8`, `nearFlat=40`, `upAbove1.01=2`.
- Score summary is `C/0=48` with max FDV `1.2494`, and `B/2=2` with max FDV
  `1.0000`; there are no C/1 rows in this cohort and no B/2 growth signal.
  `hardRejected 2x+=0`, selected Notification / HolderSnapshot totals remain
  `0 / 0`.
- Interpretation: ids `7477..7428` added useful observation depth and one
  sub-1.25x C/0 uptick, but did not reveal meaningful growth or a new 2x+
  row. This does not change the interpretation of token id `7577` and does
  not justify scoring dictionary, watchlist threshold, Notification, or
  Telegram policy changes. Another Green Metric-one resnapshot preflight is
  still worthwhile because Metric-one backlog remains high and more examples
  are needed; a later Yellow growth-report cohort/range mode remains the
  second candidate if repeated cohort analysis is the bottleneck.
- This Green turn performed no production DB write, external provider fetch,
  Telegram send, Notification mutation, Token write, Metric write,
  HolderSnapshot write, scheduler/systemd action, or rawJson dump.
...
- Counts stayed Token / Metric / Notification / HolderSnapshot
  `3383 / 1407 / 22 / 1`. Metadata status moved from
  `mint_only=2551`, `partial=819`, `enriched=13` to
  `mint_only=2501`, `partial=869`, `enriched=13`. Metric buckets stayed
  `0=2116`, `1=1180`, `2+=87`.
- All selected rows moved `mint_only -> partial` and now have
  ...
  for `2 / 50`. Score distribution is `C / 0 = 48`, `C / 1 = 2`;
  `hardRejected=0`.
- Existing Metric ids `2467..2516` remain the latest Metrics for the selected
  ...
  total stayed `0`.
- Queue after: default 24h and requested 6h windows are currently clear due
  time drift, while rolling 168h has `metricPendingCount=160`,
  `enrichPendingCount=320`, and `notifyCandidateCount=0`. Watchlist 168h has
  `13` rows, all `B / 2`, all ready, report-only. Disabled/enabled auto-send
  allowed stayed `0 / 0`, retry candidate stayed `0`, and failed
