...
- `detect:geckoterminal:new-pools --write --pumpOnly` still requires `--limit 1`, and the same pump-only gate is now allowed in watch mode; `--watch --write --pumpOnly --limit 1` is implemented but not Red-executed yet for the pump-only lowcap path
...
- then use `--pumpOnly --limit 1 --watch --write --maxIterations 1 --checkpointFile /tmp/<name>.json` on the same fixture-backed detect command only when one isolated checkpoint write should be verified without touching the default repo-local cursor path; targeted tests confirm the checkpoint advances only to the selected / processed candidate cursor and does not skip limit-out candidates
