...
- this single-mint loop and cohort reporting did not send Telegram, did not update checkpoints, did not start watch mode, and did not touch systemd; they confirm the one-shot real-data path, minimal time-series append, read-only history visibility, and cohort report visibility before automation, while write operation for multiple-token / multiple-metric cycles and watch/systemd operation remain unconfirmed
...
- Gecko detect always-on work has not started: the pump.fun one-shot write gate, same-mint one-shot observation loop with two Metrics, and read-only multi-token Metric cohort reporting are confirmed, but detect watch write, detect foreground/tmux operation, detect systemd operation, and metric snapshot watch write remain unconfirmed
...
- `tokens:compare-report` does not yet include the `metrics:report` safe summary columns for latestMetric market-data presence; adding latestMetric-level `priceUsdPresent`, `fdvUsdPresent`, `reserveUsdPresent`, and `topPoolPresent` there is the next small read-only compare improvement if cohort-level market-data presence becomes an operating need
