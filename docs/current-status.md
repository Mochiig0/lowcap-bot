...
- The detect foreground bounded watch wrapper has now passed its first bounded
  ...
  `6MD8...pump` through first Metric append.
- The first foreground-created mint,
  ...
  systemd. The first foreground-created mint remains unchanged at
  `metricsCount=2` with latestMetric `id=1129`. The next Green gate for
  `6MD8...pump` is rawJson-free report confirmation for Metric `id=1130`.
...
- Always-on monitoring is still not implemented: there is no scheduler, queue
  ...
  confirmation for `6MD8...pump` Metric `id=1130`, a read-only preflight before
  any second `6MD8...pump` Metric append, a detect tmux bounded preflight, or
