...
- The detect foreground bounded watch wrapper has now passed its first bounded
  ...
  tmux, systemd, and journal operations were not invoked. The next Green gate
  for these two new mints is read-only preflight before any enrich/rescore.
- The first foreground-created mint,
  ...
  output did not expose Metric rawJson and did not write to DB. The other foreground-created
  mint, `6MD8LtMX1Jf7W9hDs8rnthkeFS2sonzSaYiQHkZgpump`, remains untouched at
  `mint_only` with no name / symbol, no Metrics, and no latestMetric. The next
  Green gate for `5vLb...pump` is second Metric append preflight.
...
- Always-on monitoring is still not implemented: there is no scheduler, queue
  ...
  for every Red step. After that, the next candidates are second Metric append
  preflight for `5vLb...pump`, a read-only preflight before `6MD8...pump`
  enrich/rescore, a detect tmux bounded preflight, or formalizing metric
  snapshot tmux bounded operation as an interim entrypoint. Systemd stays on
  hold until a user-systemd-capable environment exists, and unbounded watch
  remains prohibited.
