...
- `token:compare -- --mint ...` can still show latestMetric and `recentMetrics`, but it includes rawJson in the current output, so regular operator reports should prefer `metrics:report`, `token:show`, and `tokens:compare-report` for rawJson-free checks and use `token:compare` only as an internal summarized view until it has safe summary output
