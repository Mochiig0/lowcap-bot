...
- this single-mint loop and cohort reporting did not send Telegram and did not touch systemd; they confirm the one-shot real-data path, the initial pump-only detect watch write gate with an isolated `/tmp` checkpoint, the watch-detected mint's first downstream enrich/rescore plus Metric append, rawJson-free report / compare confirmation for that watch-detected mint, the watch-detected mint's second Metric append / time-series behavior, minimal time-series append, bounded single-mint watch write, bounded batch watch write with one eligible token, foreground bounded watch natural exit with minGap skip, tmux bounded watch natural exit with one append plus one minGap skip, tmux bounded no-candidate natural exit with zero writes, read-only history visibility, and cohort report visibility before automation, while the watch-detected mint's two-Metric read-only report check, foreground append, two-or-more-token simultaneous Metric write, unbounded watch, restart-oriented operation, and systemd operation remain unconfirmed
...
- for the Gecko pump.fun detect watch path, keep using `--pumpOnly --limit 1 --watch --write --maxIterations 1 --checkpointFile /tmp/<name>.json` for isolated checks; the first live Red gate passed with one mint ingest write and one `/tmp` checkpoint update, while targeted tests confirm the checkpoint advances only to the selected / processed candidate cursor and does not skip limit-out candidates
...
- Gecko detect always-on work has not started: the pump.fun one-shot write gate, initial pump-only detect watch write gate with `/tmp` checkpoint, watch-detected mint follow-up through enrich/rescore plus first Metric append, first rawJson-free report confirmation, and second Metric append, same-mint one-shot observation loop with two Metrics, bounded single-mint and batch Metric snapshot watch writes, bounded foreground/tmux Metric snapshot gates, and read-only multi-token Metric cohort reporting are confirmed, but detect watch write beyond the first isolated gate, detect foreground/tmux operation, detect systemd operation, the watch-detected mint's two-Metric read-only report check, two-or-more-token simultaneous metric snapshot write, long-running metric snapshot watch, restart-oriented metric snapshot operation, and metric snapshot systemd operation remain unconfirmed
