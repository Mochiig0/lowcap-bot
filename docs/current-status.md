...
- Always-on monitoring is still not implemented: there is no scheduler, queue
  ...
  and an explicit `--maxIterations`, keep enrich/rescore and Metric appends as single-mint
  ...
  for the two newly created foreground-detect mints, a detect tmux bounded
  preflight, or formalizing metric snapshot tmux bounded operation as an
  interim entrypoint. Systemd stays on hold until a
