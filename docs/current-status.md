...
- `token:compare -- --mint ...` now shows latestMetric and `recentMetrics` with rawJson-free `safeSummary` booleans, so the same single-token time-series view can be used in operator reports without pasting rawJson
