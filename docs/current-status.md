...
- `pnpm review:pending-shape:geckoterminal -- --sinceHours 24 --limit 10` is the smaller read-only helper for SMOKE-excluded recent Gecko-origin pending rows, with lightweight `metadataStatus`, `selectionAnchorKind`, `reviewFlagsCount`, and `queuesMatched` pattern summaries plus a short representative-row preview
