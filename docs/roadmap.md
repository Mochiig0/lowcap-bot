...
`7527..7478` and returned `selected=50`, `ok=50`, `written=50`,
`skipped=0`, `error=0`, and `providerErrorCount=0`. It wrote Metric ids
`2667..2716` with observedAt range
`2026-06-09T09:25:10.363Z..2026-06-09T09:37:58.148Z`; all selected rows
moved from Metric-one to Metric>=2. Counts moved only in Metric:
`4065 / 1557 / 22 / 1 -> 4065 / 1607 / 22 / 1`, and global Metric buckets
moved to `0=2748`, `1=1080`, `2+=237`.

Post-run `metrics:growth-report` now evaluates `235` pumpOnly Metric>=2 rows.
...
fallback command, and rawJson full dump all stayed closed. Next lane should be
...
That Green growth review is now complete. ids `7477..7428` are all Metric>=2
and rawJson-safe to inspect, but the cohort did not produce a growth winner.
Top cohort FDV multiple is `1.2494`, FDV
`>=1.1/>=1.25/>=1.5/>=2/>=3/>=5/>=10` is `1/0/0/0/0/0/0`, and latest FDV
shape is `40` near flat, `8` down, and `2` up above `1.01x`. Score buckets
are `C/0=48` and `B/2=2`; the B/2 rows have max FDV `1.0000`, and there are
no C/1 rows in this cohort.

Policy remains unchanged: token id `7577` is still the only meaningful growth
signal, one isolated C/1 winner is not enough for scoring or watchlist
changes, and `notifyCandidate=0` keeps Notification / Telegram closed.
Recommended next slice: **Green Metric-one resnapshot preflight** to look for
more examples while the Metric-one backlog remains high. Second candidate:
**Yellow growth-report cohort/range enhancement** if ad hoc cohort review
keeps recurring.

That next Green preflight is now complete. The baseline
`metrics:growth-report` remained safe and read-only, with pumpOnly Metric>=2
evaluated rows `285`, top FDV/reserve `3.8445 / 3.7064`, FDV
`2x/3x/5x/10x=1/1/0/0`, `C/1` as the only 2x+ score bucket, `B/2` max
`1.0058`, and token id `7577` still the isolated meaningful growth signal.

The fetch-free `--onlyMetricOnce` preview selected the next clean cohort:
ids `7427..7378`, `selectedCount=50`, all `metricsCount=1`, latest Metric
ages `8325..8338` minutes, selected Notification / HolderSnapshot totals
`0 / 0`, and `providerErrorCount=0`. There is no overlap with previous
Metric-one resnapshot cohorts `8259..8210`, `7577..7528`, `7527..7478`, or
`7477..7428`; 1440 and 720 minute windows selected `0`.

Recommended next slice: **human-approved network-enabled Metric-one
resnapshot Red** for ids `7427..7378`, followed by `metrics:growth-report`
