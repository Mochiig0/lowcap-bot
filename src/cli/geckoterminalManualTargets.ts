...
function buildManualTargetItem(
  token: SelectedToken,
  ...
  if (
    token.metadataStatus !== "partial" ||
    !queuesMatched.includes("staleReview") ||
    !queuesMatched.includes("metricPending")
  ) {
    return null;
  }
...
async function run(): Promise<void> {
  ...
  const manualTargetItems = smokeExcludedTokens
    .map((token) => buildManualTargetItem(token, staleAfterHours))
    .filter((item): item is ManualTargetItem => item !== null);
  const selectedRows = manualTargetItems.slice(0, args.limit);
    ...
    JSON.stringify(
      {
        ...
        selection: {
          ...
          eligibleManualTargetCount: manualTargetItems.length,
