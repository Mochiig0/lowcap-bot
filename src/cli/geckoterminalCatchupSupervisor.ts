...
export type Args = {
  ...
  opsNotifyRequested: boolean;
  opsNotifyTrigger: OpsNotificationTrigger | null;
...
type OpsNotifyPlan = {
  enabled: boolean;
  channel: "telegram";
  delivery: "preview_only" | "capture_only" | "send" | "capture_and_send";
  sendRequested: boolean;
  sendSupported: boolean;
  selectedTrigger: OpsNotificationTrigger | null;
  sentCount: number;
  sendResults: OpsNotificationSendResult[];
...
function getUsageText(): string {
  return [
    "Usage:",
    "pnpm ops:catchup:gecko -- [--write] [--metricAppend] [--pumpOnly] [--limit <N>] [--maxCycles <N>] [--sinceMinutes <N> | --sinceHours <N>] [--dry-run] [--captureFile <PATH>] [--opsNotifyCaptureFile <PATH>] [--opsNotify] [--opsNotifyTrigger token_completed|metric_appended|loop_complete] [--cooldownSeconds <N>] [--stopOnNotifyCandidate <true|false>] [--stopOnRateLimit <true|false>]",
    "",
    "Defaults:",
...
function parseOpsNotifyTriggerArg(value: string): OpsNotificationTrigger {
  if (
    value === "token_completed" ||
    value === "metric_appended" ||
    value === "loop_complete"
  ) {
    return value;
  }

  throw new CliUsageError(`Invalid --opsNotifyTrigger: ${value}`);
}
...
export function parseGeckoCatchupSupervisorArgs(argv: string[]): Args {
  ...
  const out: Args = {
    ...
    opsNotifyRequested: false,
    opsNotifyTrigger: null,
    captureFile: null,
  ...
  for (let i = 0; i < normalizedArgv.length; i += 1) {
    ...
    switch (key) {
      ...
      case "--opsNotifyCaptureFile":
        ...
        break;
      case "--opsNotifyTrigger":
        out.opsNotifyTrigger = parseOpsNotifyTriggerArg(value);
        break;
...
function buildOpsNotifyPlan(output: Omit<GeckoCatchupSupervisorOutput, "opsNotifyPlan">): OpsNotifyPlan {
  ...
  const captureRequested = output.selection.opsNotifyCaptureFile !== null;
  const sendRequested = output.selection.opsNotifyRequested;

  return {
    enabled: sendRequested,
    channel: "telegram",
    delivery:
      captureRequested && sendRequested
        ? "capture_and_send"
        : captureRequested
          ? "capture_only"
          : sendRequested
    ...
    sendRequested,
    sendSupported: false,
    selectedTrigger: output.selection.opsNotifyTrigger,
    sentCount: 0,
    sendResults: [],
    captureSupported: true,
    captureRequested,
...
function withBlockedOpsNotificationSend(input: {
  output: GeckoCatchupSupervisorOutput;
  ...
  return {
    ...input.output,
    opsNotifyPlan: {
      ...input.output.opsNotifyPlan,
      sendSupported: input.sendSupported ?? input.output.opsNotifyPlan.sendSupported,
      sentCount: 0,
      sendResults: [
        {
...
async function sendAndMarkMetricAppendedNotification(
  output: GeckoCatchupSupervisorOutput,
  deps: GeckoCatchupSupervisorDeps,
): Promise<GeckoCatchupSupervisorOutput> {
  const selectedCaptureResults = output.opsNotifyPlan.captureResults.filter(
    (result) => result.trigger === "metric_appended",
  );
  const selectedCaptureResult = selectedCaptureResults[0] ?? null;

  if (
    selectedCaptureResults.length !== 1 ||
    selectedCaptureResult === null ||
    selectedCaptureResult.status !== "captured"
  ) {
    return withBlockedOpsNotificationSend({
      output,
      trigger: "metric_appended",
      mint: selectedCaptureResult?.mint ?? null,
      metricId: selectedCaptureResult?.metricId ?? null,
  ...
  const blockedBy = [
    ...(selectedCaptureResult.mint === null ? ["mint_missing"] : []),
    ...(selectedCaptureResult.metricId === null ? ["metric_id_missing"] : []),
    ...(deps.notificationClient === undefined ? ["notification_client_not_connected"] : []),
  ];

  if (
    blockedBy.length > 0 ||
    selectedCaptureResult.mint === null ||
    selectedCaptureResult.metricId === null ||
    deps.notificationClient === undefined
  ) {
    return withBlockedOpsNotificationSend({
      output,
      trigger: "metric_appended",
      mint: selectedCaptureResult.mint,
      metricId: selectedCaptureResult.metricId,
      blockedBy,
      sendSupported: deps.opsNotifySender !== undefined,
    });
  }
  ...
  const notification = await findNotificationByKey(
  ...
  if (result?.status === "sent") {
    await markNotificationSent(deps.notificationClient, notificationKey);
  } else if (result?.status === "failed") {
    await markNotificationFailed(deps.notificationClient, notificationKey, {
      errorCode: result.errorCode ?? "ops_notify_sender_failed",
...
export async function sendOpsNotifyPlan(
  ...
  if (
    notificationRecordOutput.opsNotifyPlan.sendRequested &&
    notificationRecordOutput.opsNotifyPlan.selectedTrigger === "metric_appended"
  ) {
    return sendAndMarkMetricAppendedNotification(notificationRecordOutput, deps);
  }
...
export function buildGeckoCatchupSupervisorPlan(
  ...
  return attachOpsNotifyPlan({
    ...
    selection: {
      ...
      opsNotifyRequested: args.opsNotifyRequested,
      opsNotifyTrigger: args.opsNotifyTrigger,
      cooldownSeconds: args.cooldownSeconds,
      stopOnNotifyCandidate: args.stopOnNotifyCandidate,
      stopOnRateLimit: args.stopOnRateLimit,
      metricAppendRequested: args.metricAppendRequested,
...
export async function runGeckoCatchupSupervisor(
  ...
  const captureOutput = await captureOpsNotifyPlan(metricAppendOutput);
  return sendOpsNotifyPlan(captureOutput, deps);
}
