...
function getUsageText(): string {
  return [
    ...
    "",
    "Defaults:",
    `- compares ${COMPARED_SOURCES.length} GeckoTerminal token snapshot endpoints`,
...
function parseAvailability(raw: unknown): ParsedAvailability {
  if (!isRecord(raw)) {
...
function buildFetchUrl(source: ComparedSource, mint: string): string {
...
async function fetchTokenSnapshotRaw(source: ComparedSource, mint: string): Promise<unknown> {
  const explicitFixturePath = process.env[source.fixtureEnvVar];
  const fallbackFixturePath = process.env.GECKOTERMINAL_TOKEN_SNAPSHOT_FILE;
  const fixturePath = explicitFixturePath ?? fallbackFixturePath;
  ...
  const response = await fetch(buildFetchUrl(source, mint), {
    headers: {
...
function updateSummaryFromAvailability(
...
async function run(): Promise<void> {
  ...
  for (const token of selection.selectedTokens) {
    ...
    for (const source of COMPARED_SOURCES) {
      ...
      try {
        const raw = await fetchTokenSnapshotRaw(source, token.mint);
  ...
  const output: Output = {
    ...
    availabilitySummary: COMPARED_SOURCES.map((source) => summaries.get(source.id) ?? buildEmptySummary(source)),
    sampleResults,
  };
